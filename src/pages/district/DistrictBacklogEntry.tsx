...
export default function DistrictBacklogEntry() {
    ...
    const loadInitialData = async () => {
      ...
      try {
        const [booksResponse, savedEntriesResponse] = await Promise.all([
          booksAPI.getAll(),
          stockAPI.getAll({ type: "DISTRICT", userId: "1601" }),
        ]);
  ...
  const handleBookSelect = (rowId: string, bookId: string) => {
    ...
    if (existingEntry) {
      const shouldEdit = window.confirm(
        `This book "${selectedBook?.title}" already exists in your saved district stock with quantity ${existingEntry.quantity}. ` +
      ...
      if (shouldEdit) {
        ...
        setRows((prev) => [
          ...prev,
          {
            ...
            type: "DISTRICT",
            userId: "DISTRICT-1", // Adjust this as needed
  ...
  const handleSave = async (id: string) => {
    ...
    if (row.isNew) {
      ...
      if (existingEntry) {
        alert(
          `This book "${row.book?.title}" already exists in your saved district stock. Please edit the existing entry instead of creating a new one.`,
        );
        return;
      }
  ...
  const handleSaveAll = async () => {
    ...
    for (const row of newRows) {
      ...
      if (existingEntry) {
        alert(
          `Book "${row.book?.title}" already exists in your saved district stock. Please remove it from the current rows or edit the existing entry instead.`,
        );
        return;
      }
    }
