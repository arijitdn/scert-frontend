...
export default function DistrictReceived({
  adminLevel,
}: {
  adminLevel: string;
}) {
  ...
  // Get available eChallan numbers that are delivered to district
  const getAvailableEChallans = () => {
    const deliveredEChallans = echallans.filter(
      (challan) =>
        challan.status === "DELIVERED" &&
        challan.destinationType === "DISTRICT",
    );

    console.log("Total eChallans:", echallans.length);
    console.log("Delivered to District eChallans:", deliveredEChallans.length);
      ...
      if (challan.books && challan.books.length > 0) {
        challan.books.forEach((book) => {
          ...
          const stockEntry = stock.find(
            (s) => s.bookId === book.bookId && s.type === "DISTRICT",
          );
  ...
  const handleBookChange = (value: string) => {
    ...
    if (book) {
      ...
      const existingStock = stockData.find(
        (stock) => stock.bookId === book.bookId && stock.type === "DISTRICT",
      );
  ...
  const handleSaveStockEntry = async () => {
    ...
    try {
      ...
      const existingStock = stockData.find(
        (stock) =>
          stock.bookId === stockEntryData.bookId && stock.type === "DISTRICT",
      );
      ...
      if (existingStock) {
        ...
        const stockEntry = {
          ...
          quantity: stockEntryData.received,
          userId: "DISTRICT_ADMIN", // Default district admin user ID
          type: "DISTRICT",
        };
      ...
      try {
        const stockResponse = await stockAPI.getAll({ type: "DISTRICT" });
        setStockData(stockResponse.data.data || []);
      } catch (refreshError) {
  ...
  if (loading) {
    return (
      <AdminLayout
        title="District Books Received"
        description="Loading district stock data..."
        adminLevel={adminLevel || "DISTRICT ADMIN"}
      >
  ...
  return (
    <AdminLayout
      title="District Books Received"
      description="Manage books received at district level from eChallans"
      adminLevel={adminLevel || "DISTRICT ADMIN"}
    >
      {/* Statistics Cards */}
      ...
      <Card className="w-full max-w-5xl mx-auto mb-8 bg-gradient-to-br from-blue-100 to-blue-50 border-blue-300">
        <CardHeader>
          ...
          <CardDescription>
            Enter details for books received from eChallans at district level.
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card className="w-full max-w-6xl mx-auto bg-gradient-to-br from-purple-100 to-pink-50 border-purple-300">
        <CardHeader>
          ...
          <CardDescription>
            Overview of all books received at district level from eChallans
          </CardDescription>
        </CardHeader>
        <CardContent>
